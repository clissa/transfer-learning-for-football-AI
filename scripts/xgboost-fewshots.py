...
def _plot_fewshot_curves(
    ...
    """Save a 2×2 panel of few-shot learning curves."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    for ax, (metric_key, metric_label, higher_better) in zip(axes, metrics_to_plot):
        # Source-only horizontal line
        src_val = source_metrics.get(metric_key, np.nan)
        ax.axhline(src_val, color="#2ca02c", ls="--", lw=1.8,
                    label=f"Source-only ({src_val:.3f})")
        ...
        for scenario, color in palette.items():
            ...
            nice = "Target-only" if scenario == "target_only" else "Fine-tune"
            ax.plot(xs, means, marker="o", color=color, lw=2, label=nice)
...
def _print_and_save_table(
    ...
    # Source-only row
    row: dict[str, Any] = {"Model": "Source-only (no adapt)", "Budget target": "0%"}
    for m in display_metrics:
        row[m.upper() if m != "pr_auc" else "PR-AUC"] = f"{source_metrics.get(m, np.nan):.4f}"
    # Fix naming
    row_renamed: dict[str, Any] = {}
    for k, v in row.items():
        new_k = k.replace("roc_auc", "ROC-AUC").replace("brier", "BRIER").replace("f1", "F1").replace("logloss", "LOGLOSS").replace("pr_auc", "PR-AUC")
        row_renamed[new_k] = v
    ...
    for budget in budgets_pct:
        ...
        for scenario in ["target_only", "finetune"]:
            ...
            if sub.empty:
                continue
            nice = "Target-only" if scenario == "target_only" else "Fine-tune (xgb_model)"
            r: dict[str, Any] = {"Model": nice, "Budget target": budget_label}
            for m in display_metrics:
...
def main() -> int:
    ...
    # Align target columns to match source model features
    ...
    logger.info(
        "Source-only (zero-shot) on target: PR-AUC=%.4f  Brier=%.4f  F1=%.4f",
        source_metrics['pr_auc'], source_metrics['brier'], source_metrics['f1'],
    )
