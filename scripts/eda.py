...
KNOWN_ID_COLS = {
    "game_id", "original_event_id", "action_id",
}
KNOWN_META_COLS = {
    "competition_name", "country_name", "competition_gender", "season_name",
    "game_date", "venue", "referee", "player_name", "nickname", "team_name",
    "starting_position_name", "competition_stage",
}
LEAKAGE_SUSPECT_COLS = {
    "home_score", "away_score", "scores", "concedes",
}
...
TRAINING_COMPETITIONS   = ["La Liga", "Premier League", "1. Bundesliga"]
...
if leakage:
    ...
    print("\n⚠ Leakage suspects:")
    for item in leakage:
        print(f"  {item['column']:25s}  {item['reason']:25s}  corr={item['corr_with_target']:.4f}")
else:
...
if NUM_FEATS:
    ...
    plot_univariate_numeric_grid(
        df, NUM_FEATS, OUTPUT_DIR / "univariate_numeric.png", seed=SEED,
    )
...
if CAT_FEATS:
    ...
    for col in CAT_FEATS:
        ...
        _cat_records.append({
            "column": col, "n_unique": int(df[col].nunique(dropna=True)),
            "top_value": top_val, "top_pct": top_pct,
    ...
    plot_univariate_categorical_grid(
        df, CAT_FEATS, OUTPUT_DIR / "univariate_categorical.png", top_k=TOP_K_CAT,
    )
...
plot_bivariate_target_corr(biv_df, TARGET_COL, OUTPUT_DIR / "bivariate_target_corr.png", top_k=TOP_K_BIV)
    ...
    for i in range(len(_cols)):
        for j in range(i + 1, len(_cols)):
            ...
            if abs(r) >= CORR_THRESHOLD:
                _pairs.append({"col_a": _cols[i], "col_b": _cols[j], "pearson_r": round(r, 4)})
...
if ALL_FEATS:
    ...
    scaler, encoder, all_feature_names = build_preprocessor(
        X_tr, num_feats=NUM_FEATS, cat_feats=CAT_FEATS, min_frequency=10,
    )
    ...
    log.info(
        ...
        n_obs, n_cols, len(NUM_FEATS), n_cols - len(NUM_FEATS),
    )

    baseline_metrics_rows: list[dict[str, float | str]] = []

    def _register(model_name: str, split_name: str, y_true: np.ndarray, y_proba: np.ndarray) -> dict:
        ...
        log.info(
            ...
            model_name, split_name, m["roc_auc"], m["pr_auc"],
    ...
    lr_metrics_val = _register("LogReg", "validation", y_val, lr.predict_proba(X_val_preproc)[:, 1])
    lr_metrics_test = _register("LogReg", "test", y_te, lr.predict_proba(X_te_preproc)[:, 1])

    lr_imp = pd.Series(np.abs(lr.coef_[0]), index=all_feature_names).sort_values(ascending=False)
    ...
    rf_metrics_val = _register("RandomForest", "validation", y_val, rf.predict_proba(X_val)[:, 1])
    rf_metrics_test = _register("RandomForest", "test", y_te, rf.predict_proba(X_te)[:, 1])
    ...
    mlp = MLPClassifier(
        ...
        validation_fraction=0.1, n_iter_no_change=10, max_iter=200,
        random_state=SEED,
    )
    ...
    baseline_metrics_df.to_csv(OUTPUT_DIR / "baseline_metrics_by_split.csv", index=False)
    ...
    for split_name in ("validation", "test"):
        ...
        print(subset[["model", "roc_auc", "pr_auc", "precision", "recall", "f1"]].to_string(index=False))
